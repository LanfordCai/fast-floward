...
transaction(recipient: Address, metadata: {String: String}) {
    ...
    // the tenant
    let tenant: &RegistryNFTContract.Tenant{RegistryNFTContract.ITenant}
    let receiver: &RegistryNFTContract.Collection{NonFungibleToken.CollectionPublic}

    prepare(acct: AuthAccount) {

        self.tenant = acct.borrow<&RegistryNFTContract.Tenant{RegistryNFTContract.ITenant}>(from: RegistryNFTContract.TenantStoragePath)
